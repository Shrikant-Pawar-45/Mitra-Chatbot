...
def synthesize_voice(text: str, gender: str) -> tuple[bytes, str, str]:
    """Return (audio_bytes, mime, filename). Uses pyttsx3 for gendered voices if available, else gTTS."""
    # Try offline pyttsx3 first for real male/female selection
    res = _synthesize_with_pyttsx3(text, gender)
    if res is not None:
        return res
    # Fallback to gTTS (no gender control)
...
with tab_voice:
    ...
    if pyttsx3 is None and gTTS is None:
        st.warning("No TTS engine available. Install either pyttsx3 (offline) or gTTS.")
    text = st.text_area("Text to speak", "Hello! This voice was generated by Mitra.")
    col_m, col_f = st.columns(2)
    disabled_btns = (pyttsx3 is None and gTTS is None) or not text.strip()
